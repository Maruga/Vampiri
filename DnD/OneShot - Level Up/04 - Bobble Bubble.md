# LIVELLO 3 — BUBBLE BOBBLE

![[04 - Bobble Bubble.png]]

## Mappa

- **Dimensioni:** 28 x 40 quadretti (1 quadretto = 5' = 1,5m)
- **Origine:** in basso-destra
- **Ingresso:** Alto (centro-alto)
- **Uscita:** Basso (una delle due porte centrali)
- **Passaggi Segreti:** 3 
- **Livello illuminato**

## Scalare i Muri (Livello 3)
> [!warning] Muri Scalabili
> A differenza dei livelli precedenti, i muri di questo livello possono essere scalati. I PG potrebbero tentare di evitare l'acqua passando sopra i muri. Questo è permesso, ma è rischioso: superficie ghiacciata, vento forte, nessuna copertura, e Aphros che vola sopra vede tutto. Il ladro ha un vantaggio, gli altri faticano. Non vietare — lascia che provino e scoprano da soli quanto è pericoloso.

### Condizioni per Scalare

| Requisito               | Dettaglio                                      |
| ----------------------- | ---------------------------------------------- |
| **Ladro**               | Può scalare usando il suo modificatore abilità |
| **Altri PG**            | Devono avere chiodi + martello o simile        |
| **Piantare chiodo**     | Azione gratuita                                |
| **Velocità con chiodi** | 1□ per round                                   |
| **Altezza muri**        | 4 quadretti (20')                              |
| **TS durante scalata**  | Richiesto ogni round o caduta                  |
### Scalata e Caduta
**Scalata:** 1□ per round, 3 TS richiesti (al 1°, 2° e 3° quadretto). Al 4° sei in cima.
**Armatura pesante:** -2 al TS durante la scalata.

| Caduta da...  | Altezza | Danno |
| ------------- | ------- | ----- |
| 1° quadretto  | 5'      | 1d6   |
| 2° quadretto  | 10'     | 1d6   |
| 3° quadretto  | 15'     | 2d6   |
| Cima del muro | 20'     | 2d6   |
### Sollevare un PG con la Corda
Molto complicato — superficie scivolosa, nessun appiglio. Chi tiene la corda deve fare TS Paralisi ogni round o perde la presa. Se perde la presa, chi sta scalando cade.

| Elemento                                | Regola                                       |
| --------------------------------------- | -------------------------------------------- |
| **Chi tira**                            | Prova FOR ogni round (1d20, risultato ≤ FOR) |
| **Velocità**                            | 1□ per round                                 |
| **Fallimento**                          | Entrambi cadono                              |
| **Armatura pesante (chi viene tirato)** | -4 alla prova FOR di chi tira                |

### Movimento in Cima al Muro

| Chi                  | Movimento         | Tiro richiesto                            |
| -------------------- | ----------------- | ----------------------------------------- |
| **Ladro**            | Metà movimento    | Nessuno                                   |
| **Ladro**            | Movimento normale | TS Paralisi o cade                        |
| **Altri PG**         | Max 1□ per round  |                                           |
| **Altri PG**         | Movimento>2□      | TS Paralisi o cade                        |
| **Armatura pesante** | Max 1□ per round  | TS Paralisi ogni movimento, anche 1 passo |

### Condizioni in Cima

| Elemento | Descrizione |
|---|---|
| Superficie | Neve e ghiaccio, sconnessa |
| Larghezza | 5' (1□) |
| Vento | Forte, costante |
| Copertura | Nessuna — completamente esposti |

### Caduta

| Elemento      | Effetto                                |
| ------------- | -------------------------------------- |
| Danni         | Standard: 1d6 per 10' (max 2d6 da 20') |
| Direzione     | 1d6: pari = destra, dispari = sinistra |
| Cade in acqua | Meccanica corrente normale + Tiberinus |
### Magia in Cima

| Situazione           | Effetto                          |
| -------------------- | -------------------------------- |
| Lanciare incantesimo | TS Incantesimi per concentrarsi  |
| Fallimento TS        | Incantesimo perso, slot bruciato |
### Pericoli Esterni

| Minaccia | Effetto |
|---|---|
| **Sulis** (arciere) | Bersaglio esposto, +2 TxC contro chi è in cima |
| **Fons** (spintore) | Può spingere giù, TS Paralisi per resistere |
| **Aphros** (volante) | Vede immediatamente chi scala, allerta tutti |

### Nota sulla Mappa

Il muro in fondo (basso-destra) arriva fino in cima. La tana di Aphros, in basso a destra zona illuminata con uscita, il fantasmino volante esce da quella posizione.


## Ingresso nel Livello

> _Leggere quando i PG attraversano il portale:

> Il gelo svanisce. L'aria è umida, pesante. Sentite il suono prima di vederlo — un gorgoglio costante, ritmico, come il respiro di qualcosa di enorme. L'odore di muschio bagnato e sale antico vi riempie le narici.
> Alzate lo sguardo. Non c'è soffitto — solo un cielo notturno infinito, punteggiato di stelle che brillano di luce fredda. È quella luce a illuminare tutto, riflessa dall'acqua e dal ghiaccio.
> I muri che vi circondano sono diversi da quelli dei livelli precedenti. Ghiaccio antico — blu scuro, quasi nero, liscio come vetro e duro come pietra. Oltre di essi sentite il vento ululare, trascinando neve e schegge di ghiaccio. Qualcosa di primordiale, che esisteva prima del Natale.
> Acqua. Un fiume che scende dall'alto, serpeggiando tra piattaforme di pietra dorata. La corrente è visibile — bolle che scoppiano in superficie, schiuma che si accumula agli angoli.
> E in quell'acqua... qualcosa si muove. Qualcosa vi aspetta.
> 
> **LIVELLO 3 — START**

![[01 Introduction.wav]]

## Mappa — Legenda

|Elemento|Descrizione|
|---|---|
|**Corrente (azzurra)**|Fiume che scorre dall'alto verso il basso|
|**Piattaforme (sabbia)**|Terreno solido, sicuro|
|**Muri (arancione)**|Indistruttibili|
|**Passaggi Segreti ($)**|2 necessari per raggiungere l'uscita|
|**Regali (x6)**|Sparsi nella mappa|
|**Uscita**|Una delle due porte in basso al centro|

# MECCANICA CORRENTE

## Come Funziona

1. Il PG entra nell'acqua
2. Tira **TS Paralisi**
3. **Successo:** attraversa senza problemi
4. **Fallimento:** viene trascinato verso il basso

## Forza della Corrente
Le frecce indicano la direzione e la forza da quel punto in poi.

| Simbolo | Forza  | Spostamento per round   |
| ------- | ------ | ----------------------- |
| ◇       | Debole | 3 quadretti (15'/4,5m)  |
| ◇◇      | Media  | 6 quadretti (30'/9m)    |
| ◇◇◇     | Forte  | 9 quadretti (45'/13,5m) |

## Regole Trascinamento

- **Ogni round** il PG può tentare un nuovo TS Paralisi per uscire
- **Fallimento:** continua a scendere dello spostamento indicato
- **Impatto contro muro:** 1d6 danni
- **Mentre trascinato:** non può lanciare incantesimi, combattere o compiere altre azioni
- **Movimento laterale:** il PG viene spostato seguendo il percorso della corrente, viene sempre attirato verso i muri.

> _Quando un PG viene trascinato:_ L'acqua vi afferra come una mano gelida. Non potete respirare, non potete pensare — solo il ruggito della corrente nelle orecchie mentre vi trascina verso il basso. Il mondo diventa un turbine di bolle e freddo.


# MOSTRI

## I 4 Fantasmini — Boss del Livello

Spiriti di festività legate all'acqua e ai fiumi sacri. **Unici — se muoiono, restano morti.**

### Statistiche Base (tutti tranne dove specificato)

| Stat            | Valore                                                                    |
| --------------- | ------------------------------------------------------------------------- |
| Dadi Vita       | 5                                                                         |
| Punti Ferita    | 25                                                                        |
| Classe Armatura | 5 [14]                                                                    |
| Movimento       | 40'/12m/8 quadretti                                                       |
| Tiri Salvezza   | **TS G5:** Morte 10, Bacchette 11, Paralisi 12, Soffio 13, Incantesimi 14 |
| Morale          | 9                                                                         |
| Speciale        | Incorporei — armi normali: danno dimezzato                                |


### 🔴 TIBERINUS — Il Rosso (ACQUA)

![[3-1-TIBERINUS.png]]

**Festività:** Dio del Tevere (romano) — padre del fiume sacro di Roma. I romani gettavano offerte nelle sue acque per placare la sua furia. La festa Tiberinalia celebrava il suo potere sulle piene.

**Posizione iniziale:** Nell'acqua (centro della corrente)

**Restrizione:** **Non può uscire dall'acqua** — si muove solo nel fiume

**Comportamento:** Quando un PG tocca l'acqua, si muove verso di lui e attacca. Se il PG si allontana dall'acqua scappa, può attaccare un PG che è adiacente all'acqua ma tendenzialmente lo fa solo quando il PG è dentro l'acqua. Gli altri PG possono colpirlo a distanza.

|Stat|Valore|
|---|---|
|Movimento (in acqua)|60'/18m/12 quadretti|
|Attacco|Presa delle Profondità 1d8|
|Effetto|Bersaglio -2 al prossimo TS Paralisi per uscire dalla corrente|

> _Prima apparizione:_ L'acqua si increspa. Una forma rossa emerge — non sale, si manifesta. Come se il fiume stesso prendesse coscienza. Occhi come gorghi, braccia come correnti. È il fiume. E il fiume vi vuole.

---

### 🩷 SULIS — La Rosa (ARCIERE)

![[3-2-SULIS.png]]

**Festività:** Dea delle acque termali (celtico-romana) — venerata a Bath, dove i fedeli gettavano maledizioni incise su piombo nelle fonti sacre. Colpiva da lontano, invisibile, attraverso l'acqua.

**Posizione iniziale:** Lato sinistro, in basso (fuori dall'acqua)

**Restrizione:** **Non può attraversare l'acqua**

**Comportamento:** Spara frecce da distanza. Se i PG si avvicinano, fugge. Se intrappolata, combatte debolmente.

| Stat               | Valore                                               |
| ------------------ | ---------------------------------------------------- |
| Attacco (distanza) | Freccia Maledetta 1d6, gittata 120'/36m/24 quadretti |
| Attacco (mischia)  | Tocco Debole 1d4, **-2 TxC**                         |
| Speciale           | Fugge se PG entro 10'/3m/2 quadretti                 |

> _Prima apparizione:_ Un sibilo. Una freccia di luce rosa sfiora la vostra guancia — da dove? La vedete, lontana, dall'altra parte dell'acqua. Pallida, eterea, l'arco già teso per un altro tiro. Non vi raggiungerà mai. Ma le sue frecce sì.

---

### 🩵 FONS — Il Ciano (SPINTORE)

![[3-3-FONS.png]]

**Festività:** Dio delle sorgenti (romano) — festa Fontinalia, quando si gettavano ghirlande nei pozzi. Le sorgenti erano bocche degli dei, e Fons decideva chi beveva e chi annegava.

**Posizione iniziale:** Lato destro, in alto (fuori dall'acqua)

**Restrizione:** **Non può attraversare l'acqua**

**Comportamento:** Spinge o tira i PG verso l'acqua. Debole in corpo a corpo.

|Stat|Valore|
|---|---|
|Raggio Spinta/Tiro|40'/12m/8 quadretti|
|Spostamento|4 quadretti (20'/6m) **sempre verso l'acqua**|
|TS per resistere|TS Paralisi|
|Attacco (mischia)|Tocco Debole 1d4, **-2 TxC**|

**Meccanica:** Fons può spingere (se il PG è tra lui e l'acqua) o tirare (se l'acqua è tra lui e il PG). La direzione è sempre verso l'acqua.

> _Prima apparizione:_ Sentite una pressione. Non fisica — più profonda. Come se qualcosa vi tirasse dall'interno. Poi lo vedete: ciano pallido, braccia protese verso di voi. Non vi tocca. Ma sentite l'acqua chiamarvi. Attirarvi. E lui sorride.

---

### 🟠 APHROS — L'Arancione (CURATORE)

![[3-4-APHROS.png]]

**Festività:** Spirito della schiuma marina (greco) — "aphros" significa schiuma, da cui nacque Afrodite. Gli antichi credevano che le bolle contenessero anime, e che la schiuma potesse guarire o maledire. Le bolle erano messaggeri tra mondi.

**Posizione iniziale:** Vola liberamente

**Restrizione:** **Non può curare attraverso i muri**

**Comportamento:** Non attacca mai (tranne se costretto). Vola verso gli alleati in difficoltà e li cura con bolle luminose.

|Stat|Valore|
|---|---|
|Movimento (volo)|50'/15m/10 quadretti|
|Cura|1d8 HP per round (un alleato)|
|Attacco (solo se costretto)|Tocco Debole 1d4, **-2 TxC**|
|Speciale|Può essere colpito da frecce e magie a distanza|

**Meccanica:** Quando un alleato subisce danno, Aphros vola verso di lui e inizia a curare. Priorità: alleato con meno HP.

> _Prima apparizione:_ Bolle. Decine di bolle che salgono nell'aria — ma non dall'acqua. Da una forma arancione che fluttua sopra di voi, le ali come schiuma solidificata. Non vi guarda. Guarda i suoi fratelli. E quando uno di loro sanguina... le bolle diventano luce.

---

# PASSAGGI SEGRETI

## Posizioni

|Passaggio|Posizione|Porta a|
|---|---|---|
|**$1**|Vicino al regalo basso-sinistra|Accesso alla zona uscita|
|**$2**|Dentro la stanza basso-destra|Accesso alla zona uscita|

**Trovare i passaggi:** Percezione o dichiarazione esplicita ("cerco passaggi segreti in questa zona")

> _Quando trovano un passaggio:_ Le vostre dita sfiorano la pietra — e la pietra cede. Non si apre, si dissolve. Come una bolla che scoppia. Oltre, un corridoio nascosto. La via per l'uscita.

---

# TIMER DEL LIVELLO

|Fase|Tempo|
|---|---|
|Esplorazione libera|45 minuti reali|
|Rintocco (ondata finale)|Fine dei 45 minuti|
|Fuga verso l'uscita|15 minuti reali|

### Cosa Succede al Rintocco

> _Leggere quando scade il tempo:_
> 
> L'acqua ruggisce, si espande di un quadretto a round. La corrente accelera — tutto diventa più veloce, più forte.
> 
> E i fantasmini? Non più tattiche. Non più attese.
> 
> **Vi vogliono nell'acqua. Tutti.**

![[02 Hurry.wav]]

**Meccanica:**

- La corrente aumenta di 1 livello ovunque (max ◇◇◇)
- Tutti i fantasmini convergono verso i PG
- Morale sale a 12
- L'uscita brilla — visibile da ovunque
- PG finiscono fuori dalla mappa con l'acqua muoiono.

---

# USCITA DAL LIVELLO

> _Leggere quando attraversano l'uscita:_
> 
> La porta si chiude alle vostre spalle. Il rumore dell'acqua svanisce.
> 
> Silenzio. Freddo. Ma diverso da prima.
> 
> Sentite qualcosa davanti a voi. Qualcosa di grande. Di antico. Di affamato.
> 
> **BOSS FINALE — READY**

---

# NOTE PER IL MASTER

## Ordine di Gioco (Round)

1. **PG si muovono** (attenzione all'acqua!)
2. **PG in acqua tirano TS Paralisi**
3. **Trascinamento** (se fallito)
4. **PG agiscono** (se non in acqua)
5. **Fantasmini agiscono** (secondo comportamento)
6. **Aphros cura** (se necessario)

## Consigli

- **L'acqua è il vero nemico** — i fantasmini la sfruttano
- **Tiberinus + Fons = combo mortale** — Fons spinge, Tiberinus aspetta
- **Aphros è la priorità** — finché vive, gli altri durano di più
- **Sulis è fragile** — ma può fare danni da lontano, i PG devono raggiungerla
- **I passaggi segreti** — senza di loro, non possono uscire

## Bilanciamento

|Mostro|Minaccia|Note|
|---|---|---|
|Tiberinus|Alta (in acqua)|Innocuo fuori dall'acqua|
|Sulis|Media|Fastidiosa, facile da uccidere se raggiunta|
|Fons|Alta|Il setup per Tiberinus|
|Aphros|Bassa (diretta)|Alta (indiretta) — tiene in vita gli altri|

---

# SCHEDA RAPIDA — STAMPA E GIOCA

## Corrente

|Frecce|Spostamento|Note|
|---|---|---|
|◇|3□ (15'/4,5m)|Debole|
|◇◇|6□ (30'/9m)|Media|
|◇◇◇|9□ (45'/13,5m)|Forte|

**TS Paralisi** per attraversare o uscire | Fallimento = trascinato | Muro = 1d6 danni | No azioni mentre trascinato

## Fantasmini (HD 5 | HP 25 | CA 5 [14])

|Nome|Colore|Pos.|Movimento|Attacco|Speciale|
|---|---|---|---|---|---|
|**Tiberinus**|🔴|Acqua|60'/12□ (solo acqua)|1d8|-2 al prossimo TS corrente|
|**Sulis**|🩷|SO|40'/8□|Freccia 1d6 (24□) / Mischia 1d4 (-2)|Fugge se PG vicini, no attraversa acqua|
|**Fons**|🩵|NE|40'/8□|Spinta 8□ → 4□ verso acqua / Mischia 1d4 (-2)|TS Paralisi resiste, no attraversa acqua|
|**Aphros**|🟠|Vola|50'/10□|Mischia 1d4 (-2) solo se costretto|Cura 1d8/round, no attraverso muri|

**Speciale:** Incorporei — armi normali: danno dimezzato | TS G5 | Morale 9

## Passaggi Segreti

- **$1:** Basso-sinistra (vicino regalo)
- **$2:** Dentro stanza basso-destra

## Timer

| Fase            | Tempo  | Effetto             |
| --------------- | ------ | ------------------- |
| Esplorazione    | 45 min | —                   |
| Rintocco → Fuga | 15 min | Corrente +1 livello |